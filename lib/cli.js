...
// src/lexer.ts
var PUNCT, CONTINUATION_START, CONTINUATION, isIdentStart, isIdentChar, IDENT, Scanner;
var init_lexer = __esm({
  "src/lexer.ts"() {
    ...
    CONTINUATION_START = /\?|:|&&|\|\||\.(?=[A-Za-z_])/;
    CONTINUATION = new RegExp("^(?:\\n[ \\t]*)+(" + CONTINUATION_START.source + ")");
...
var RESERVED, BIN_PREC, ASSIGN_OPS, GT_CONTINUES, ExprParser;
var init_expr = __esm({
  "src/expr.ts"() {
    "use strict";
    init_diagnostics();
...
var VOID_ELEMENTS, DECL_LINE, FOR_LINE, IF_LINE, LINE_SHAPES, CODE_LINE, TAG_NAME, ATTR_NAME, DocParser;
var init_parser = __esm({
  "src/parser.ts"() {
    ...
    DECL_LINE = /^(?:export\s+)?(?:async\s+)?(?:var|let|prop|fn)\s/;
    FOR_LINE = new RegExp(`^for\\s+${IDENT.source}(?:\\s*,\\s*${IDENT.source})?\\s+in\\s.+\\{\\s*$`);
    IF_LINE = /^if\s+.+\{\s*$/;
    LINE_SHAPES = [DECL_LINE, FOR_LINE, IF_LINE, /^\}/, /^else\b/, /^<[A-Za-z]/, /^<\/[A-Za-z]/];
    CODE_LINE = new RegExp("^(?:" + LINE_SHAPES.map((r) => r.source.slice(1)).join("|") + ")");
